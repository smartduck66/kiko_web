...
switch (myArgs[0]) {
    case 'mf':
        ...
        var _loop_1 = function (i) {
            var filename_1 = ref.refcli[i].ref + ".data";
            console.log("Chargement de la fiche climatique de la ville : " + ref.refcli[i].town);
            var url_1 = "https://donneespubliques.meteofrance.fr/FichesClim/FICHECLIM_" + filename_1;
            var request_1 = https.get(url_1);
            request_1.on("response", function (response) {
                ...
                response.on("end", function () {
                    if (httpStatus === 200) {
                        fs.writeFileSync('../ficheclim/' + filename_1, body); // Création du fichier brut, mode texte, sur disque
                    }
                    else {
    ...
    case 'clim':
        ...
        var _loop_2 = function (i) {
            var text = fs.readFileSync("../ficheclim/" + ref.refcli[i].ref + ".data", "utf8");
    ...
    case 'immo':
        ...
        var url = "https://static.data.gouv.fr/resources/demandes-de-valeurs-foncieres/20211020-111113/valeursfoncieres-2020.txt";
        var filename_2 = "../data_source/valeursfoncieres-2020.txt";
        var request_2 = https.get(url);
        request_2.on("response", function (response) {
            var httpStatus = response.statusCode;
            response.setEncoding("utf-8");
            var body = "";
            response.on("data", function (chunk) { body += chunk; });
            response.on("end", function () {
                if (httpStatus === 200) {
                    ...
                    // Surface terrain
                    fs.writeFileSync(filename_2, body); // Création du fichier brut, mode texte, sur disque
                    fs.appendFileSync(filename_2, "|||||||999999|99/99/9999|Vente|99999999,99|99||RUE|9999||99999||99||||||||||||||||9|9|||||||999"); // Guard à la fin du fichier
                }
                else {
                    new Error('HTTP status ${response.statusCode}');
                }
            });
            request_2.on("error", function (error) {
                console.log(error);
            });
        });
        ...
        var lineReader = require('readline').createInterface({
            input: require('fs').createReadStream(filename_2)
        });
        lineReader.on('line', function (line_read) {
            if (num_line_1 > 0) { // on saute la 1ère ligne du fichier
